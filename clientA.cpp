...
#define CENTRAL_SERVERC_PORT "25408" //define portnumber
#define ERROR_FLAG -1
...
int main() {
	...
	//set the TCP connect: learn from Beej’s socket programming tutorial 
	...
	/**********************************************************************/		
	    ...
	    if (recv(sockfd, clientBMsg, sizeof(clientBMsg), 0) == ERROR_FLAG) {
			perror("recv");
		    exit(1);
		}
	...
	/**********************************************************************/
        ...
        if (final_result[0] == '/'){
			cout << "Found no compatibility for " << clientInput << " and " << clientBMsg <<endl;
		//return 0;
		    break;
		}
		memset(buf, '\0', sizeof(buf));
		...
		/**********************************output result*********************************/
        /*********************************************************************************/
        cout << "Found compatibility for " << path1[0] << " and " << path1[a-1] << ":" << endl;
	    for (int i=0; i<a-1; i++){
			cout << path1[i] << " --- ";
		}
	    cout << path1[a-1]<< endl;
